...
//        return button
//    }()
    let createSocWalletButton: UIButton = {
        let button = Button(size: .large, style: .solid)
        button.translatesAutoresizingMaskIntoConstraints = false
        button.setTitle(NSLocalizedString("welcome.createSocWallet.button.title", value: "CREATE SOC WALLET", comment: ""), for: .normal)
        button.titleLabel?.font = UIFont.systemFont(ofSize: 16, weight: UIFont.Weight.semibold)
        button.backgroundColor = Colors.darkBlue
        return button
    } ()
...
//            createWalletButton.trailingAnchor.constraint(equalTo: stackView.trailingAnchor),
            ...
            createSocWalletButton.leadingAnchor.constraint(equalTo: stackView.leadingAnchor),
            createSocWalletButton.trailingAnchor.constraint(equalTo: stackView.trailingAnchor),
...
//        createWalletButton.addTarget(self, action: #selector(start), for: .touchUpInside)
        createSocWalletButton.addTarget(self, action: #selector(addSOC), for: .touchUpInside)
